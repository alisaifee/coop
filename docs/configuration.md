...
| `config_dir` | string (path) or `false` | `~/.grok` | Source directory for Grok Build files. Copies an allowlist of entries (`AGENTS.md`, `auth.json`, `lsp.json`, `rules/`, `skills/`, `commands/`, `plugins/`, `hooks/`, `agents/`, `workflows/`) from this directory to `~/.grok/` in the guest on start. Host `config.toml` is merged into the guest file (except `[plugins]`), not copied over it. Directory symlinks, hidden directories (`.git`, `.venv`, caches), and bare git repos (`*.git`) under those entries are skipped. `installed-plugins/` is a host-absolute registry and stays on the host. Set to `false` to disable. Supports `~` expansion. |
